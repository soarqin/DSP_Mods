...
public class Miner : PatchImpl<Miner>
{
    ...
    private static readonly ThreadLocal<uint> MiningSeed = new(() => (uint)Random.Range(0, int.MaxValue));
    ...
    private static void GameData_GameTick_Prefix()
    {
        ...
        if (main.timei % 60 == 0)
        {
            ...
            _frameCounter = Mathf.RoundToInt(_frame / 1200000f);
            _frame -= _frameCounter * 1200000f;
        }
        else
        {
            _frameCounter = 0;
        ...
        for (var factoryIndex = data.factoryCount - 1; factoryIndex >= 0; factoryIndex--)
        {
            ...
            foreach (var (itemIndex, itemId) in VeinList)
            {
                foreach (var storageIndex in demands[itemIndex])
                {
                    ...
                    if (itemIndex > 0)
                    {
                        (amount, energyConsume) = Mine(factory, veins, itemId, miningScale, _frameCounter, station.energy);
                        if (amount < 0) continue;
                    }
                    else
                    {
                        energyConsume = (WaterEnergyConsume.Value * _frameCounter * miningScale * miningScale + 9999L) / 10000L;
    ...
    private static (int, long) Mine(PlanetFactory factory, ProductVeinData[] allVeins, int productId, int percent, int counter, long energyMax)
    {
        ...
        for (; count > 0; count--)
        {
            ...
            {
                MiningSeed.Value = (uint)((int)((ulong)((MiningSeed.Value % 2147483646u + 1) * 48271L) % 2147483647uL) - 1);
                consume = MiningSeed.Value < barrier;
            }
