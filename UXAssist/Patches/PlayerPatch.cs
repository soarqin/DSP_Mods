...
public class PlayerPatch : PatchImpl<PlayerPatch>
{
    ...
    private class EnhancedMechaForgeCountControl : PatchImpl<EnhancedMechaForgeCountControl>
    {
        ...
        private static bool UIReplicatorWindow_ChangeMultiplier_Prefix(UIReplicatorWindow __instance, RecipeProto ___selectedRecipe, MethodBase __originalMethod)
        {
            var recipe = ___selectedRecipe;
            if (recipe == null) return false;
            var multiplier = __instance.multipliers.TryGetValue(recipe.ID, out var current) ? current : 1;
            var step = VFInput.control ? 10 : VFInput.shift ? 100 : VFInput.alt ? 1000 : 1;
            var direction = __originalMethod.Name == nameof(UIReplicatorWindow.OnPlusButtonClick) ? 1 : -1;
            __instance.multipliers[recipe.ID] = Math.Max(1, Math.Min(1000, multiplier + direction * step));
            return false;
        }
    }
    ...
    {
        [HarmonyPrefix]
        [HarmonyPatch(typeof(UIGame), nameof(UIGame.OnSandCountChanged))]
        private static bool UIGame_OnSandCountChanged_Prefix()
        {
            return false;
        }
    }
